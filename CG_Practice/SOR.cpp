...
//모델링을 하기 위한 변수
GLfloat xLocation, yLocation, zLocation = 0; // location
...
//    char directory= "C:\\Users\\wisdom99\\Downloads\\Computer Graphics\\mymodel.dat";
    ...
    for (int k = 0; k < s - 1; k++) //처음기둥과 마지막 기둥
    {
        fprintf(fout, "%d %d %d\n", m - s + k + 1, m - s + k, k + 1);
        fprintf(fout, "%d %d %d\n", k, k + 1, m - s + k);
    }

    for (int k = 0; k < n - 2; k++) //밑면과 뚜껑
    {
        fprintf(fout, "%d %d %d\n", (k + 2) * s, 0, (k + 1) * s);
        fprintf(fout, "%d %d %d\n", (k + 3) * s - 1, s - 1, (k + 2) * s - 1);
    }
...
{
    printf("--------------------------------------------------\n");
    printf("마우스 왼쪽 클릭 : 점 찍기\n");
    printf("오른쪽 마우스 클릭 :메뉴 출력\n");
    printf("\n");
    printf("w >>> WireFrame \n");
